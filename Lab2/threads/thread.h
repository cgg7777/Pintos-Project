...
struct thread
  {
    ...
    bool isLoaded;		// if loaded on memory, then true
    bool isTerminated;		// if terminated, then true
    struct semaphore exit_sema;// call V() when child thread operation is end, call P() when child thread is running
    struct semaphore load_sema;// call V() when waiting load operation, call V() when load operation is end.
    int exit_status;
