...
void process_exit(void)
{
    ...
    //  printf(" **** /process.c : process_exit - thread tid : %d, lock holder tid : %d\n", thread_current()->tid, filesys_lock->holder->tid);
...
setup_stack(void **esp)
{
    ...
    if (kpage != NULL)
    {
        success = install_page(((uint8_t *)PHYS_BASE) - PGSIZE, kpage->physical_address, true);
        if (success)
            *esp = PHYS_BASE;
        else
